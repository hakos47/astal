...
function _getChildren(widget: Gtk.Widget): Array<Gtk.Widget> {
  if ("get_child" in widget && typeof widget.get_child == "function") {
    return (widget as any).get_child() ? [(widget as any).get_child()] : [];
  }
...
export default function astalify<
  ...
  Signals extends Record<`on${string}`, Array<unknown>> = Record<
    `on${string}`,
    any[]
  >,
  ...
  return {
    [cls.name]: (
      ...
      const widget = new cls(
        "cssName" in props ? { cssName: props.cssName } : {},
      );
      ...
      // REPARACIÓN 2: construct ahora recibirá un objeto con 'setup' validado en el prototipo
      return construct(widget as any, setupControllers(widget, props as any));
    },
  }[cls.name];
}

// ... (Resto de tipos y setupControllers se mantienen igual)
...
type EventController<Self extends Gtk.Widget> = {
  ...
  onHoverLeave?: (self: Self, x: number, y: number) => void;
...
function setupControllers<T>(
  ...
  if (onFocusEnter || onFocusLeave) {
    ...
    if (onFocusEnter) focus.connect("enter", () => onFocusEnter(widget));

    if (onFocusLeave) focus.connect("leave", () => onFocusLeave(widget));
  }
